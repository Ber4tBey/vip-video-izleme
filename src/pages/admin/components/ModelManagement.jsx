...
const EMPTY = { name: '', bio: '', file: null, previewUrl: '' };
...
const ModelManagement = () => {
  ...
  const startEdit = (m) => {
    setForm({ name: m.name, bio: m.bio || '', file: null, previewUrl: getMediaUrl(m.image_url) || '' });
    setEditId(m.id);
  };
          ...
          models.map((m) => (
            <div key={m.id} className={`card p-4 flex items-center gap-3 ${!m.is_active ? 'opacity-50' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                {m.bio && <p className="text-gray-500 text-xs truncate">{m.bio}</p>}
              </div>
