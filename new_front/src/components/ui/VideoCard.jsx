...
const VideoCard = ({ video, showViewCount = true }) => {
  ...
  const previewSrc = video.preview_url ? getMediaUrl(video.preview_url) : '';
  const thumbnailSrc = getMediaUrl(video.thumbnail_url);

  // Hover handlers live on the aspect-video parent so they work above all z-index overlays
  const handleMouseEnter = () => setHovering(true);
  const handleMouseLeave = () => setHovering(false);
  const handleTouchStart = () => {
    longPressRef.current = setTimeout(() => setHovering(true), 400);
  };
  ...
  const inner = (
    <div className={`card group cursor-pointer hover:border-dark-400 hover:shadow-xl hover:shadow-black/40 transition-all duration-300 hover:-translate-y-1 ${!canWatch ? 'opacity-90' : ''}`}>
      ...
      <div
        className="relative overflow-hidden aspect-video bg-dark-600"
        onMouseEnter={previewSrc ? handleMouseEnter : undefined}
        onMouseLeave={previewSrc ? handleMouseLeave : undefined}
        onTouchStart={previewSrc ? handleTouchStart : undefined}
        onTouchEnd={previewSrc ? handleTouchEnd : undefined}
        onTouchCancel={previewSrc ? handleTouchEnd : undefined}
      >
        {/* Thumbnail image (always visible, fades on hover if preview exists) */}
        <VideoThumbnail
          thumbnail={thumbnailSrc}
          className={`absolute inset-0 w-full h-full transition-transform duration-500 ${hovering ? '' : 'group-hover:scale-105'}`}
          alt={video.title}
          loading="lazy"
        />

        {/* Preview video — mounts only on hover */}
        {previewSrc && hovering && (
          <video
            key={previewSrc}
            src={previewSrc}
        ...
        {/* Play overlay on hover — hidden while preview is playing */}
        {canWatch && !hovering && (
