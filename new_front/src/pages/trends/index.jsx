...
const TrendsPage = () => {
  ...
  const fetchTrends = useCallback(async (p) => {
    ...
    try {
      if (p === 1) {
        // Fetch top 3 separately (first page includes them)
        const top3Res = await api.get('/videos?sort=views&limit=3&page=1');
        setTop3((top3Res.videos || []).slice(0, 3));
      }
      // Fetch rest: offset by 3 for proper ranking
      const offset3Page = p;
      const restRes = await api.get(`/videos?sort=views&limit=${PER_PAGE}&page=${offset3Page}`);
      
      // For page 1, skip first 3 (they're in top3 podium)
      const restVideos = p === 1 
        ? (restRes.videos || []).slice(3)
        : (restRes.videos || []);
      
      setVideos(restVideos);
      
      const restTotal = Math.max(0, (restRes.total || 0) - 3);
      setTotal(restTotal);
      setTotalPages(Math.ceil(restTotal / PER_PAGE));
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
